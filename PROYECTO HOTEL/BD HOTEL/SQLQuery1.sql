...
AS
SELECT * FROM CLIENTES
WHERE CLI_PAIS LIKE @TEXTOBUSCAR + '%'
...
--MUESTRA HABITACIONES DE PRECIO MAXIMO
CREATE PROCEDURE HAB_MOSTRAR_PRECIO_MAX
AS
SELECT* FROM HABITACIONES
WHERE HAB_PRECIO_DIARIO=
(select max(HAB_PRECIO_DIARIO)
from HABITACIONES)
...
@FECHA_FIN DATE
AS
SELECT DISTINCT RES_ID, HAB_NUM AS #HABITACION,HABITACIONES.HAB_TIPO, COSTO_NOCHES AS PRECIO, RESERVA.ESTADO_HAB, CLIENTES.CLI_NOMBRES AS NOMBRES, CLIENTES.CLI_APELLIDOS AS APELLIDOS, CHECKIN, CHECKOUT, N_NOCHES AS NOCHES, COSTO_TOTAL AS TOTAL, FECHA_RESERVACION, ESTADO_PAGO, OBSERVACIONES
FROM RESERVA JOIN CLIENTES ON RESERVA.CLI_ID=CLIENTES.CLI_ID JOIN HABITACIONES ON RESERVA.HAB_ID=HABITACIONES.HAB_ID
WHERE CHECKIN >= @FECHA_INICIO AND CHECKIN < @FECHA_FIN  OR CHECKOUT <= @FECHA_FIN AND CHECKOUT > @FECHA_INICIO
...
AS
SELECT TOP 50 HAB_NUM AS #HABITACION,HABITACIONES.HAB_TIPO, COSTO_NOCHES AS PRECIO, RESERVA.ESTADO_HAB, CLI_NOMBRES AS NOMBRES, CLI_APELLIDOS AS APELLIDOS, CHECKIN, CHECKOUT, N_NOCHES AS NOCHES, COSTO_TOTAL AS TOTAL, FECHA_RESERVACION, ESTADO_PAGO, OBSERVACIONES
from RESERVA, HABITACIONES, CLIENTES
WHERE  HABITACIONES.HAB_ID= RESERVA.HAB_ID AND CLIENTES.CLI_ID=RESERVA.CLI_ID
